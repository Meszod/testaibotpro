"""
╔══════════════════════════════════════════════════════════════╗
║           PROFESSIONAL QUIZ BOT  —  Global Edition          ║
║   Yaratuvchi: Sultonboyev Muhammad  |  @mesz0d               ║
╚══════════════════════════════════════════════════════════════╝
"""
import asyncio, json, re, os, uuid, logging, io
...
# ═══════════════════════════════════════════════════════════════
# ⚠️  MAXFIY MA'LUMOTLAR — endi muhit o'zgaruvchilaridan olinadi!
# Railway "Variables" bo'limida quyidagilarni kiriting:
#   BOT_TOKEN, GROQ_API_KEY, ADMIN_IDS, ADMIN_USERNAME, BOT_USERNAME
# ═══════════════════════════════════════════════════════════════
...
if not GROQ_API_KEY:
    raise RuntimeError("GROQ_API_KEY muhit o'zgaruvchisi topilmadi.")
if not ADMIN_IDS:
    raise RuntimeError("ADMIN_IDS muhit o'zgaruvchisi topilmadi (masalan: 8517530604).")

# DB fayli joylashadigan papkani yaratib qo'yamiz (Railway Volume /data ga bog'lansa saqlanadi)
...
async def register_user(uid, first_name, last_name, level):
    async with aiosqlite.connect(DB_FILE) as db:
        await db.execute("""
            UPDATE users SET first_name=?,last_name=?,level=?,registered=1
            WHERE user_id=?
...
async def is_registered(uid) -> bool:
    async with aiosqlite.connect(DB_FILE) as db:
        async with db.execute(
            "SELECT registered FROM users WHERE user_id=?", (uid,)
        ) as cur:
...
async def get_stats(uid):
    async with aiosqlite.connect(DB_FILE) as db:
        async with db.execute(
            "SELECT correct,total FROM users WHERE user_id=?", (uid,)
        ) as cur:
...
async def save_test(test_id, owner_id, title, data):
    async with aiosqlite.connect(DB_FILE) as db:
        await db.execute("""
            INSERT OR REPLACE INTO tests(test_id,owner_id,title,data_json)
            VALUES(?,?,?,?)
...
async def get_referral_count(uid: int) -> int:
    async with aiosqlite.connect(DB_FILE) as db:
        async with db.execute(
            "SELECT COUNT(*) FROM referrals WHERE owner_id=?", (uid,)
        ) as cur:
...
async def add_referral(owner_id: int, referred_user: int):
    async with aiosqlite.connect(DB_FILE) as db:
        async with db.execute(
            ...
            if not exists:
                await db.execute("""
                    INSERT INTO referrals(owner_id, referred_user)
                    VALUES(?, ?)
                """, (owner_id, referred_user))
                await db.commit()
        ...
        try:
            await bot.send_message(
                ...
                reply_markup=InlineKeyboardBuilder().button(
                    ...
                    url=f"https://t.me/{username[1:]}" if username != "-" else None
                ).as_markup() if username != "-" else None
            )
            log.info(f"Admin {admin_id} ga xabar yuborildi (user: {uid})")
        except TelegramForbiddenError:
            log.warning(f"Admin {admin_id} botni bloklagan!")
        except TelegramBadRequest as e:
            log.warning(f"Admin {admin_id} ga xabar yuborishda xato: {e}")
...
def parse_questions(text: str) -> list:
    ...
    for b in blocks:
        ...
        if q and len(opts) >= 2:
            questions.append({"question": q.group(1).strip(),
                               "options": opts[:4], "correct": cidx})
    return questions
    ...
    if args.startswith("ref_"):
        ...
        try:
            ...
            if owner_id != uid:
                ...
                log.info(f"Referral qo'shildi: {owner_id} -> {uid}")
        except ValueError:
...
async def cb_send_to_group(c: CallbackQuery):
    ...
    try:
        await c.message.edit_text(
            "💬 <b>Qaysi guruhga test yubormoqchisiz?</b>\n\n"
            "Pastdan tanlang 👇",
            reply_markup=group_list_kb(groups, test_id)
        )
    except TelegramBadRequest:
        await c.message.answer(
            "💬 <b>Qaysi guruhga test yubormoqchisiz?</b>\n\n"
...
async def cb_to_send(c: CallbackQuery):
    ...
    try:
        ...
        try:
            await c.message.edit_text(
                f"✅ <b>Test muvaffaqiyatli yuborildi!</b>\n\n"
                f"💬 Guruh: {chat_id}\n📝 {len(tests)} ta savol",
                reply_markup=home_kb()
            )
        except TelegramBadRequest:
...
async def main():
    await init_db()
    log.info("=" * 50)
    log.info("  ✅ Quiz Bot Professional ishga tushdi!")
    log.info("=" * 50)
